...
const Calendar: FC<Props> = ({ value = new Date(), onChange }) => {
  ...
  return (
    <div className="w-[400px]">
      <div className="grid grid-cols-7 items-center justify-center text-center">
        <Cell onClick={prevYear}>{"<Y"}</Cell>
        <Cell onClick={prevMonth}>{"<M"}</Cell>
        <Cell className="col-span-3">{format(value, "LLLL yyyy")}</Cell>
        <Cell onClick={nextMonth}>{"M>"}</Cell>
        <Cell onClick={nextYear}>{"Y>"}</Cell>
        {weeks.map((week) => (
